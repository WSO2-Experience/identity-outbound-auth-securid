...
public class RSASecurIdAuthenticator extends AbstractApplicationAuthenticator
    ...
    protected void initiateAuthenticationRequest(HttpServletRequest request,
        ...
        if (authenticatedUser == null) {
            throw new AuthenticationFailedException
                    ("Authentication failed!. Cannot proceed further without identifying the user",authenticatedUser);
        }
        try {
    ...
    protected void processAuthenticationResponse(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, AuthenticationContext authenticationContext) throws AuthenticationFailedException {
        ...
        try {
            ...
            throw new AuthenticationFailedException("Error occurred while loading user realm or user store manager : " + e.getMessage(), authenticatedUser);
        }
        ...
        if (StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(passCode)) {
            try {
                ...
                throw new AuthenticationFailedException("Cannot Create the API : " + e.getMessage(),authenticatedUser);
            }finally {
                if(api != null)
                    try {
                        api.shutdown();
                    } catch (AuthAgentException e) {
                        throw new AuthenticationFailedException("Could not able to shut downn the API : " +  e.getMessage(),authenticatedUser);
                    }
            }
        } else {
            throw new AuthenticationFailedException("UserID & Password are Empty",authenticatedUser);
        }
    }
